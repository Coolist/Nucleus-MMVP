...
var run = function() {
  ...
  $('.add.device').click(function() {
    $(this).fadeOut();
    $('.home-devices').transition({
      height: 0
    });
    $('body, html').transition({ scrollTop: $('.home-header').outerHeight() });
  });
  ...
  $('.add-trigger').click(function() {
    $('.trigger-examples').css('opacity', 0).transition({
      opacity: 1,
      height: 'toggle'
    });
    $('body, html').transition({ scrollTop: $('.trigger-examples').offset().top });
    step = 1;
  });
  ...
  $('.trigger-examples .item').click(function() {
    if (step === 1) {
      ...
      $('.trigger-examples').transition({
        opacity: 0,
        height: 'toggle'
      });
      $('.add-action').css('opacity', 1);
      $('body, html').transition({ scrollTop: $('.add-trigger').offset().top - 120 });
      step = 2;
    }
  });
  ...
  $('.add-action').click(function() {
    if (step === 2) {
      $('.action-examples').css('opacity', 1).transition({
        opacity: 1,
        height: 'toggle'
      });
      $('body, html').transition({ scrollTop: $('.action-examples').offset().top });
      step = 3;
    }
  });
  ...
  $('.action-examples .item').click(function() {
    if (step === 3) {
      ...
      $('.action-examples').transition({
        opacity: 0,
        height: 'toggle'
      });
      $('body, html').transition({ scrollTop: $('.trigger-example').offset().top - 120 });
  ...
  $('.restart').click(function() {
    if (step === 4) {
      ...
      $('.trigger-example').transition({
        opacity: 0,
        height: 'toggle'
      });
      $('.add-trigger').css('opacity', 0).transition({
        opacity: 1,
        height: 'toggle'
      });
      $('.add-action').css('opacity', 0).transition({
        opacity: 0.4,
        height: 'toggle'
      });
      step = 0;
  ...
  $('#page').smoothState({
    ...
    onStart: {
      ...
      render: function (url, $container) {
          $container.transition({opacity: 0}, 200, function() {
            $('body, html').scrollTop(0);
            $container.transition({opacity: 1}, 200);
            run();
          });
        }
      }
